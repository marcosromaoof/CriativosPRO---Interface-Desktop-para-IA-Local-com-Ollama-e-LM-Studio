...
class DatabaseManager:
    def __init__(self, db_name="criativospro.db", key_name="security.key"):
        # Definir diretório de dados do usuário (APPDATA no Windows)
        # Ex: C:\Users\Nome\AppData\Roaming\CriativosPro
        app_data_dir = os.path.join(os.getenv('APPDATA'), 'CriativosPro')
        if not os.path.exists(app_data_dir):
            os.makedirs(app_data_dir)

        self.db_path = os.path.join(app_data_dir, db_name)
        self.key_path = os.path.join(app_data_dir, key_name)
        
        print(f"[Database] Usando caminho: {self.db_path}")
