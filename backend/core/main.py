...
async def sync_provider_models(sid, data):
    ...
    try:
        ...
        if not provider:
            await sio.emit("sync_error", {"message": f"Falha ao carregar driver do provedor '{provider_name}'. Verifique logs."}, to=sid)
            return
...
async def generate_tts(sid, data):
    ...
    if filename:
        ...
        audio_url = f"http://127.0.0.1:5678/audio/{filename}"
